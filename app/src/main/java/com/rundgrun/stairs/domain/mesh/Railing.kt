...
class Railing(
    ...
    override fun draw() {
        ...
        GLES20.glDrawArrays(
            GLES20.GL_TRIANGLES,
            (data.verticesBackground.size + data.verticesRung.size + data.verticesMetalProfile.size) / (POSITION_COUNT + TEXTURE_COUNT),
            data.verticesRailing.size / (POSITION_COUNT + TEXTURE_COUNT)
        )
    }
}